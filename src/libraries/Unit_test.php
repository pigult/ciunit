...
class CI_Unit_test
{
	...
	public function __construct()
	{
		...
		$this->_test_items_visible = array(
			'test_name', 'test_datatype', 'res_datatype', 'result', 'file', 'line', 'notes'
		);
	...
	function run($test, $expected = true, $test_name = 'undefined', $notes = '')
	{
		...
		$report[] = array(
			'test_name' => $test_name, 'test_datatype' => gettype($test), 'res_datatype' => $extype,
			'result' => ($result === true) ? 'passed' : 'failed', 'file' => $back['file'], 'line' => $back['line'],
			'notes' => $notes
		);
	...
	function report($result = array())
	{
		...
		if (php_sapi_name() !== 'cli') {
			...
			if (count($failed_test) > 0) {
				$r =
					$r1 . "\x1b[31m" . $r2 . "\x1b[0m" . sprintf("Failed Test: \n %s \n", implode("\n ", $failed_test));
			} else {
